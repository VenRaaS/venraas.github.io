...
function toggleBtnController(){
    ...
    if(data == 0 && menuShowController() < 150 && height < 100){
        // 如果 data 狀態是關閉，且候選區的原始高度只有一行，且沒有任何tag被選取的情況

        // 將 swipe 的狀態設定為 0 ，用以改變開關箭頭的顯示樣式
        swipe.attr('swipe-type', 0); // none
    }else if(data == 1 && menuShowController() < 150 && height < 100){
        // 如果 data 狀態是開啟，且候選區的原始高度只有一行，且沒有任何tag被選取的情況

        // data 自動轉回關閉狀態
        swipeToggle(0);
    }
    else if (data == 0 && menuShowController() > 150 && height < 100){
        // 如果 data 狀態是關閉，且候選區的原始高度大於一行，且沒有任何tag被選取的情況

        // 將 swipe 的狀態設定為 1 ，用以改變箭頭得顯示樣式
        swipe.attr('swipe-type', 1); // up-blue
    ...
    if(data == 0 && selectedShowController() > 150){
        ...
        $('#ST').append($('<div/>', {
            ...
            style: 'position: absolute; right: 0; top: 0; padding: 3px 8px;',
            text: '•••'
        }).on('click', function(){swipeToggle(1); $('.top-more-btn').remove(); swipe.attr('swipe-type', 3);}));
        // ????? max-height還沒定義
        // ????? 觸發機制是什麼？
    }
}
