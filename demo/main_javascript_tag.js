...
$(function(){
    ...
    $(' #rec_pos_form , #device_form , #advenced_option_form, #advenced_option_form_2').change(function(){
...
function getGoodsKeyword() {
    ...
    p.gid = (document.getElementById("gid").value) ? document.getElementById("gid").value : '5742934';
...
function nameSelect() {
    ...
    if (target.hasClass('off')) {
        ...
        TweenMax.to(cloneTarget, 0.08, {scale:0.8, yoyo:true, repeat:1, onComplete:function(){
          ...
          0 > scrollAmount ? scrollAmount = 0 : scrollAmount = scrollAmount;
          // alert($('#tagON').width() + ',' + cloneTarget.width() + ',' + scrollAmount);
          tagsScroller(scrollAmount);
        }});
        TweenMax.set(star, {width:'19px', scale:1, opacity:1});
...
function process_item(addr, img, name, price, i) {
    ...
    var html = '<div style="margin: 0 10px 10px 10px;" hintIndex="' + i + '">';
    html += '<a class="slick-num'+ i +'" onclick="dev_func(this)" style="width: 100%; text-align: center;"><img src="' + img + '" style="width: 100%; max-width: 180px;"></a><div><a class="href-a" target="_blank" href="' + addr + '"><span style="margin: 5px 0;font-size: ' + title_size + 'px;height: ' + parseInt(title_size * 3) + 'px;display: block;overflow: hidden;word-wrap: break-word;word-break: break-all;color:' + title_color +';">' + name + '</span></div><div style="font-size: ' + price_size + 'px;font-weight: 900;color:' + price_color + ';text-align:center;"><span style="font-weight: normal;font-size: ' + price_sign_size + 'px; color:' + price_sign_color + ';">$</span><span style="font-size: ' + price_size + '; color: ' + price_color + '">' + price + '</span></a></div></div>';

    return html;
}

function dev_func(obj) {
    var clone_item = $(obj).parents('.slick-slide'),
