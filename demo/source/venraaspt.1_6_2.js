...
var venraastool = {
    ...
    doCookieSetup: function(name, value, expirestime) {
        ...
        try {
            if (expirestime == 0) {
                document.cookie = name + "=" + escape(value) + ";Path=/" + ";domain=" + window.location.host.split('.')
                    .slice(-levelth)
                    .join('.');
            }
            else {
                ...
                document.cookie = name + "=" + escape(value) + ";expires=" + expires.toGMTString() + ";Path=/" + ";domain=" + window.location.host.split('.')
                    .slice(-levelth)
                    .join('.');
                //log('debug in doCookieSetup(): done');
            }
        }
        catch (e) {}
    },
    ...
    urlparamfactory: function(_idx, obj) {
        ...
        if (_idx == venstrob.strnull) {
            for (var _k in obj) {
                str.push(encodeURIComponent(_k) + "=" + encodeURIComponent(JSON.stringify(obj[_k])));
            }
        }
        else {
            str.push(encodeURIComponent(_idx) + "=" + encodeURIComponent(JSON.stringify(obj[_idx])));
        }
        return str.join("&");
    },
    xhr: (
        ...
        function() {
            log("[xhr]window.XMLHttpRequest: " + window.XMLHttpRequest);
            log("[xhr]window.ActiveXObject: " + window.ActiveXObject);
            log("[xhr]window.XMLHttpRequest!!");
        ...
        function() {
            ...
            log("[xhr]window.ActiveXObject: " + window.ActiveXObject);
            try {
                log("[xhr]window.ActiveXObject!!");
                return new window.ActiveXObject("Microsoft.XMLHTTP");
            }
            catch (e) {
                log("[xhr]window.ActiveXObject: " + e);
            }
        }
    ),
    ...
    updateCachekeys: function(comToken, cacheKeyJson) {
        ...
        var keyInfo = {
            'key': cacheKeyJson,
            'expires_timestamp': current_timestamp + 7 * 24 * 60 * 60 * 1000
        };
...
var vencontrob = {
    ...
    autoretrieve: function(venact) {
        ...
        try {
            this.setpdata(venact, 'client_tzo', (new Date())
                .getTimezoneOffset());
            this.setpdata(venact, 'client_utc', (new Date())
                .getTime());
        }
        catch (e) {}
            ...
            if (ck_iframe === null) {
                ...
                if (typeof isetting == 'undefined' || typeof isetting.tagID == 'undefined') {
                    ...
                    if (document.getElementById(isetting.tagID) !== null) {
                        ...
                        document.getElementById(isetting.tagID)
                            .appendChild(venguid);
                    }
                    else {
