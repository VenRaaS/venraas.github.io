...
function process_result(result, div_class, loop, rowItems, showItems, scrollItems) {
  ...
  hintText = []
  for (var i = 0; i < result.recomd_list.length; i++) {
    html += process_item1(result.recomd_list[i].goods_page_url, 'http://35.201.207.10:8000' + result.recomd_list[i].goods_img_url, result.recomd_list[i].name, result.recomd_list[i].sale_price, i, result.recomd_list[i].id)
    hintText[i] = process_hintText(result.recomd_list[i].ref_item_list, result.recomd_list[i].sales)
  }
...
$(function () {
  $('#upload-form').on('submit', (function (ev) {
    ...
    $.ajax({
      ...
      url: 'http://35.201.207.10:8000/cupid/api/image/rank/' + $('#file').val().split('\\').pop(),
      type: 'POST',
      ...
      success: function (data, status, xhr) {
        ...
        let str = 'http://35.201.207.10:8000' + data.uploaded_file_url
