...
jQuery(function($){amcharts2.defaultTheme="light";var theme=amcharts2.getQueryVar("theme");if(!theme)
    ...
    $("#btn-show").on("click",function(){var action=$(this).data("type");var resources=[];for(var i=0;i<demoData.resources.length;i++){var resource=demoData.resources[i];if(resource.match(/\.css/))
    ...
    if(amcharts2.currentDemoTheme)
        $("body").removeClass("demo.js-theme-"+ amcharts2.currentDemoTheme);$("body").addClass("demo.js-theme-"+ theme);amcharts2.currentDemoTheme=theme;amcharts2.updateParamLinks();amcharts2.processLazyLoad();if(typeof demoData==="undefined")
...
amcharts2.getConfigVar=function(prop){var path=prop.split('.');var holder=AmCharts.charts[0];var element;while(element=path.shift()){var index=element.match(/\[([0-9])*\]/);if(index!==null){element=element.replace(/\[.*/,"");holder=holder[element];element=Number(index[1]);}
    ...
    return undefined;}
amcharts2.initDemoControls=function(){var $=jQuery;var controlHolder=$('#demo.js-control-holder');$(amchartsDemoControls).each(function(){var control=$("<div>").addClass("demo.js-control col-md-4");control.append($("<div>").addClass("demo.js-control-category").html(this.category));var value=amcharts2.getConfigVar(this.property);if(this.type=="slider"){control.append($("<div>").addClass("demo.js-control-value").html(value));}
    control.append($("<div>").addClass("demo.js-control-title").html(this.title));if(this.type=="slider"){var step=(this.max- this.min)/ 100;
        if(step>1)
