...
function process_slick(result, slickClass, loop, rowItems, showItems, scrollItems) {
    ...
    for (var i=0; i<result.recomd_list.length; i++) {
        ...
        cid = result.recomd_list[i].category_code;
        gid = result.recomd_list[i].id;
        from_rec = result.recomd_id;
        html += process_item(cid, gid, from_rec, goods_img_url, result.recomd_list[i].name, result.recomd_list[i].sale_price, i);
